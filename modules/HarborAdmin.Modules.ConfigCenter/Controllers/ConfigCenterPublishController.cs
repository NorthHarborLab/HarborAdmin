...
public sealed class ConfigCenterPublishController(ConfigCenterService service) : ControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyList<ConfigReleaseDto>>> ListReleases(
        ...
        Ok(await service.ListReleasesAsync(appId, environment, cancellationToken));
    ...
    public async Task<ActionResult<PublishConfigResult>> Publish(
        ...
        CancellationToken cancellationToken) =>
        Ok(await service.PublishAsync(appId, environment, request, cancellationToken));

    /// <summary>获取已发布配置快照；<paramref name="version"/> 为 0 或未传时取最新版本。</summary>
    /// <param name="appId">应用标识。</param>
    /// <param name="environment">环境名称。</param>
    ...
    {
        var snapshot = await service.GetPublishedSnapshotAsync(appId, environment, version, cancellationToken);
