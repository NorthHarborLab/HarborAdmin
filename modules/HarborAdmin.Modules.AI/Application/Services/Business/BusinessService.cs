    ...
    : HarborCrudApplicationService<AiBusiness, AiBusinessDto, PageRequest, SaveAiBusinessRequest, IAiBusinessRepository>(repository)
{
    ...
    /// 将保存请求应用到业务。
    /// </summary>
    protected override async Task<HarborResult> ApplySaveAsync(AiBusiness entity, SaveAiBusinessRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var now = UtcNow;
            ...
            entity.Routes = request.Routes
                ...
                .ToList();
            if (await Repository.BusinessKeyExistsAsync(
                    entity.BusinessKey,
                    entity.Id > 0 ? entity.Id : null,
                    cancellationToken))
            {
                return HarborResult.Failure(AiBusinessErrorCodes.DuplicateKey.Create(
                    new Dictionary<string, object?> { ["businessKey"] = entity.BusinessKey }));
            }
        ...
        {
            return HarborResult.Failure(AiBusinessErrorCodes.InvalidInput.Create(
                new Dictionary<string, object?> { ["reason"] = exception.Message }, exception.Errors, exception.ErrorMeta));
        }
    }

    /// <inheritdoc />
    protected override HarborErrorDefinition NotFoundError => AiBusinessErrorCodes.NotFound;
