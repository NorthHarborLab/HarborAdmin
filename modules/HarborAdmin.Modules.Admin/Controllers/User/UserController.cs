...
namespace HarborAdmin.Modules.Admin.Controllers.User;

/// <summary>
/// 当前用户接口（需 access token）。
/// </summary>
[ApiController]
[Route("api/user")]
    ...
    ICurrentUser currentUser) : ControllerBase
{
    ...
    /// </summary>
    [HttpGet("info")]
    public async Task<ApiResult<CurrentUserDto>> GetInfo(CancellationToken cancellationToken)
    {
    ...
    /// </summary>
    [HttpGet("codes")]
    public async Task<ApiResult<IReadOnlyList<string>>> GetCodes(CancellationToken cancellationToken)
    {
        var session = await sessionService.BuildSessionAsync(currentUser.Id, cancellationToken);
